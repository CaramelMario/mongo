...
namespace mongo {
    ...
    Status RocksCompactServerParameter::_set(int newNum) {
        auto db = _engine->getDB();
        if (newNum == 1) {
            return rocksToMongoStatus(rocksdb::experimental::SuggestCompactRange(db, nullptr, nullptr));
        }
        else if (newNum == 2) {
            static std::mutex s_mutex;
            if (s_mutex.try_lock()) {
                std::thread t([db] {
                    std::lock_guard<std::mutex> l(s_mutex, std::adopt_lock);
                    auto options = db->GetOptions();
                    rocksToMongoStatus(db->CompactRange(nullptr, nullptr, true, options.num_levels - 1));
                });
                t.detach();
            }
            return Status::OK();
        }
        return Status(ErrorCodes::BadValue, str::stream() << name() << " has to be 1 or 2");
    }

    RocksTerarkZipMinLevelServerParameter::RocksTerarkZipMinLevelServerParameter(RocksEngine* engine)
        : ServerParameter(ServerParameterSet::getGlobal(), "terarkZipMinLevel", false, true),
          _engine(engine) {}

    void RocksTerarkZipMinLevelServerParameter::append(OperationContext* txn, BSONObjBuilder& b,
                                             const std::string& name) {
        auto db = _engine->getDB();
        auto options = db->GetOptions();
        auto table_factory = options.table_factory.get();
        if (strcmp(table_factory->Name(), "TerarkZipTable") != 0) {
            b.append(name, "");
        }
        else {
            auto tzopt = (rocksdb::TerarkZipTableOptions *) table_factory->GetOptions();
            b.append(name, tzopt->terarkZipMinLevel);
        }
    }

    Status RocksTerarkZipMinLevelServerParameter::set(const BSONElement& newValueElement) {
        if (!newValueElement.isNumber()) {
            return Status(ErrorCodes::BadValue, str::stream() << name() << " has to be a number");
        }
        return _set(newValueElement.numberInt());
    ...
    Status RocksTerarkZipMinLevelServerParameter::setFromString(const std::string& str) {
        int num = 0;
        Status status = parseNumberFromString(str, &num);
        if (!status.isOK()) return status;
        return _set(num);
    ...
    Status RocksTerarkZipMinLevelServerParameter::_set(int newNum) {
        auto db = _engine->getDB();
        auto options = db->GetOptions();
        if (newNum < -1 || newNum > options.num_levels) {
            return Status(ErrorCodes::BadValue, str::stream() << name() << " has to be in -1 ~ max_level");
        }
        auto table_factory = options.table_factory.get();
        if (strcmp(table_factory->Name(), "TerarkZipTable") != 0) {
            return Status(ErrorCodes::BadValue, str::stream() << name() << " only worked on Terark");
        }
        auto tzopt = (rocksdb::TerarkZipTableOptions*)table_factory->GetOptions();
        tzopt->terarkZipMinLevel = newNum;
        return Status::OK();
    }
