...
namespace mongo {
    ...
    namespace {
        ...
        public:
            ...
            void restore() override {
                ...
                if (!_iterator.get() ||
                    ...
                    _iterator.reset(ru->NewIterator(_cfHandle, _prefix));
                    _currentSequenceNumber = ru->snapshot()->GetSequenceNumber();
        ...
        protected:
            ...
            rocksdb::ColumnFamilyHandle* _cfHandle;                 // not owned
            std::string _prefix;
        ...
        public:
            RocksStandardCursor(OperationContext* txn, rocksdb::DB* db,
                                rocksdb::ColumnFamilyHandle* cfHandle, std::string prefix,
                                bool forward, Ordering order, KeyString::Version keyStringVersion)
                : RocksCursorBase(txn, db, cfHandle, prefix, forward, order, keyStringVersion) {
                iterator();
            }
        ...
        public:
            ...
            boost::optional<IndexKeyEntry> seekExact(const BSONObj& key,
                ...
                rocksdb::Status status = RocksRecoveryUnit::getRocksRecoveryUnit(_txn)
                    ->Get(_cfHandle, prefixedKey, &_value);
    ...
    public:
        UniqueBulkBuilder(rocksdb::ColumnFamilyHandle* cfHandle, std::string prefix, Ordering ordering,
    ...
    private:
        void doInsert() {
            ...
            auto ru = RocksRecoveryUnit::getRocksRecoveryUnit(_txn);
            ru->writeBatch()->Put(_cfHandle, prefixedKey, valueSlice);
    ...
    RocksIndexBase::RocksIndexBase(rocksdb::DB* db, rocksdb::ColumnFamilyHandle* cfHandle,
    ...
    bool RocksIndexBase::isEmpty(OperationContext* txn) {
        ...
        std::unique_ptr<rocksdb::Iterator> it(ru->NewIterator(_cfHandle, _prefix));
    ...
    RocksUniqueIndex::RocksUniqueIndex(rocksdb::DB* db, rocksdb::ColumnFamilyHandle* cfHandle,
                                       std::string prefix, std::string ident,
                                       Ordering order, const BSONObj& config,
                                       std::string collectionNamespace, std::string indexName,
                                       bool partial)
        : RocksIndexBase(db, cfHandle, prefix, ident, order, config),
          _collectionNamespace(std::move(collectionNamespace)),
          _indexName(std::move(indexName)),
          _partial(partial) {}
    ...
    Status RocksUniqueIndex::insert(OperationContext* txn, const BSONObj& key, const RecordId& loc,
        ...
        auto getStatus = ru->Get(_cfHandle, prefixedKey, &currentValue);
        ...
        ru->writeBatch()->Put(_cfHandle, prefixedKey, valueVectorSlice);
        return Status::OK();
    }
    ...
    void RocksUniqueIndex::unindex(OperationContext* txn, const BSONObj& key, const RecordId& loc,
        ...
        if (!dupsAllowed) {
            if (_partial) {
                ...
                std::string val;
                auto s = ru->Get(_cfHandle, prefixedKey, &val);
                if (s.IsNotFound()) {
                    return;
                }
            ...
            _indexStorageSize.fetch_sub(static_cast<long long>(prefixedKey.size()),
                                        std::memory_order_relaxed);
            ru->writeBatch()->Delete(_cfHandle, prefixedKey);
            return;
        }
        ...
        ru->writeBatch()->Put(_cfHandle, prefixedKey, newValueSlice);
        _indexStorageSize.fetch_sub(static_cast<long long>(prefixedKey.size()),
                                    std::memory_order_relaxed);
    }
    ...
    std::unique_ptr<SortedDataInterface::Cursor> RocksUniqueIndex::newCursor(OperationContext* txn,
                                                                             bool forward) const {
        return stdx::make_unique<RocksUniqueCursor>(txn, _db, _cfHandle, _prefix, forward, _order,
                                                    _keyStringVersion);
    }
    ...
    Status RocksUniqueIndex::dupKeyCheck(OperationContext* txn, const BSONObj& key,
        ...
        auto ru = RocksRecoveryUnit::getRocksRecoveryUnit(txn);
        std::string value;
        auto getStatus = ru->Get(_cfHandle, prefixedKey, &value);
        if (!getStatus.ok() && !getStatus.IsNotFound()) {
            return rocksToMongoStatus(getStatus);
        } else if (getStatus.IsNotFound()) {
    ...
    SortedDataBuilderInterface* RocksUniqueIndex::getBulkBuilder(OperationContext* txn,
                                                                 bool dupsAllowed) {
        return new RocksIndexBase::UniqueBulkBuilder(_cfHandle, _prefix, _order, _keyStringVersion,
                                                     _collectionNamespace, _indexName, txn,
                                                     dupsAllowed);
    }

    /// RocksStandardIndex
    RocksStandardIndex::RocksStandardIndex(rocksdb::DB* db, rocksdb::ColumnFamilyHandle* cfHandle,
                                           std::string prefix, std::string ident,
                                           Ordering order, const BSONObj& config)
        : RocksIndexBase(db, cfHandle, prefix, ident, order, config),
    ...
    Status RocksStandardIndex::insert(OperationContext* txn, const BSONObj& key,
        ...
        ru->writeBatch()->Put(_cfHandle, prefixedKey, value);
    ...
    void RocksStandardIndex::unindex(OperationContext* txn, const BSONObj& key, const RecordId& loc,
        ...
        _indexStorageSize.fetch_sub(static_cast<long long>(prefixedKey.size()),
                                    std::memory_order_relaxed);
        if (useSingleDelete) {
            ru->writeBatch()->SingleDelete(_cfHandle, prefixedKey);
        } else {
            ru->writeBatch()->Delete(_cfHandle, prefixedKey);
        }
    }
    ...
    std::unique_ptr<SortedDataInterface::Cursor> RocksStandardIndex::newCursor(
            ...
            bool forward) const {
        return stdx::make_unique<RocksStandardCursor>(txn, _db, _cfHandle, _prefix, forward, _order,
                                                      _keyStringVersion);
    }
