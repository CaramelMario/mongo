...
namespace mongo {
    ...
    long long RocksCounterManager::loadCounter(const std::string& counterKey) {
        ...
        std::string value;
        auto s = _db->Get(rocksdb::ReadOptions(), _cfHandle, counterKey, &value);
        if (s.IsNotFound()) {
            return 0;
        }
        invariantRocksOK(s);
    ...
    void RocksCounterManager::updateCounter(const std::string& counterKey, long long count,
        ...
        if (_crashSafe) {
            int64_t storage;
            writeBatch->Put(_cfHandle, counterKey, _encodeCounter(count, &storage));
        } else {
            stdx::lock_guard<stdx::mutex> lk(_lock);
            ...
            if (!_syncing && _syncCounter >= kSyncEvery) {
                ...
                for (const auto& counter : _counters) {
                    writeBatch->Put(_cfHandle, counter.first, _encodeCounter(counter.second, &storage));
                }
                _counters.clear();
                _syncCounter = 0;
            }
    ...
    void RocksCounterManager::sync() {
        rocksdb::WriteBatch wb;
        {
            ...
            int64_t storage;
            for (const auto& counter : _counters) {
                wb.Put(_cfHandle, counter.first, _encodeCounter(counter.second, &storage));
            }
            _counters.clear();
            _syncCounter = 0;
            _syncing = true;
