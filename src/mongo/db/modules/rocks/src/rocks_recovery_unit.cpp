...
namespace mongo {
    namespace {
        ...
        public:
            ...
            PrefixStrippingIterator(rocksdb::ColumnFamilyHandle* cfHandle,
                                    std::string prefix, Iterator* baseIterator,
        ...
        private:
            ...
            void endOp() {
                ...
                if (skippedDeletionsOp >=
                    ...
                    _compactionScheduler->reportSkippedDeletionsAboveThreshold(_cfHandle, _prefix);
                }
            }
    ...
    RocksIterator* RocksRecoveryUnit::NewIterator(rocksdb::ColumnFamilyHandle* cfHandle,
        ...
        auto iterator = _writeBatch.NewIteratorWithBase(_db->NewIterator(options, cfHandle));
        auto scheduler = isOplog || rocksGlobalOptions.terarkEnable ? nullptr : _compactionScheduler;
        auto prefixIterator = new PrefixStrippingIterator(cfHandle, std::move(prefix), iterator,
                                                          scheduler,
                                                          std::move(upperBound));
    ...
    RocksIterator* RocksRecoveryUnit::NewIteratorNoSnapshot(rocksdb::DB* db,
        ...
        rocksdb::ReadOptions options;
        options.iterate_upper_bound = upperBound.get();
        auto iterator = db->NewIterator(rocksdb::ReadOptions(), cfHandle);
        return new PrefixStrippingIterator(cfHandle, std::move(prefix), iterator, nullptr,
                                           std::move(upperBound));
    }
