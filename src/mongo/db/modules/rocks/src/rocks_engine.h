...
namespace mongo {
    ...
    extern std::atomic<bool> enableOplogMinOptime;
    extern std::atomic<bool> enableOplogDeleteGuard;
    extern std::atomic<int> oplogDeleteGuardExpire;
    extern std::atomic<int> oplogSizeMulti;

    extern std::mutex oplogMutex;
    ...
    public:
        ...
        virtual long long storageSize() const override;

        virtual void* prepareInitialSync() override;

        virtual void finishInitialSync(void* prepareResult) override;
    ...
    private:
        rocksdb::Status openDB(rocksdb::Options& options,
                               std::vector<rocksdb::ColumnFamilyDescriptor>& descriptors,
                               bool readOnly, rocksdb::DB** db);

        Status _createIdent(StringData ident, BSONObjBuilder* configBuilder, bool isOplog = false);
        ...
        mutable stdx::mutex _oplogMapMutex;
        std::unordered_map<uint32_t, std::weak_ptr<RocksRecordStore>> _oplogPrefixMap;
        struct OplogCollectionInit {
    ...
    public:
        static const std::string kMetaCF;
        static const std::string kOplogCF;

        static const std::string kOplogCollectorPrefix;
        static const std::string kOplogCollectorDirty;

    private:
        std::vector<rocksdb::ColumnFamilyHandle*> _cfHandles;
        uint32_t _dataCFIndex = 0;
        uint32_t _metaCFIndex = 1;
        uint32_t _oplogCFIndex = 2;
