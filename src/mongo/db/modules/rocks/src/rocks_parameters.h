...
namespace mongo {
    ...
    // db.adminCommand({setParameter:1, rocksdbCompact: 1 / 2})
    // 1 for SuggestFullRange , 2 for FullCompact
    ...
    // This is only for Terark
    // Modify TerarkZipMinLevel
    // db.adminCommand({setParameter:1, terarkZipMinLevel: -1 ~ max_level })
    class RocksTerarkZipMinLevelServerParameter : public ServerParameter {
        MONGO_DISALLOW_COPYING(RocksTerarkZipMinLevelServerParameter);

    public:
        RocksTerarkZipMinLevelServerParameter(RocksEngine* engine);
        virtual void append(OperationContext* txn, BSONObjBuilder& b, const std::string& name);
        virtual Status set(const BSONElement& newValueElement);
        virtual Status setFromString(const std::string& str);
