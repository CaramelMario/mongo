...
namespace mongo {
    ...
    public:
        ...
        Status scheduleCompactOp(rocksdb::ColumnFamilyHandle* cfHandle,
                                 const std::string& begin = std::string(), const std::string& end = std::string(),
                                 bool rangeDropped = false,
                                 const std::function<void(bool)>& cleanup = std::function<void(bool)>());
    ...
    private:
        ...
        struct CompactOp {
            ...
            rocksdb::ColumnFamilyHandle* cfHandle;  // not owned
...
// is set.
    ...
    Status CompactionBackgroundJob::scheduleCompactOp(rocksdb::ColumnFamilyHandle* cfHandle,
                                                      const std::string& begin, const std::string& end,
                                                      bool rangeDropped, const std::function<void(bool)>& cleanup) {
        {
            stdx::lock_guard<stdx::mutex> lk(_compactionMutex);
            _compactionQueue.push_back({cfHandle, begin, end, rangeDropped, cleanup});
        }
        _compactionWakeUp.notify_one();
        return Status::OK();
    }
    ...
    void CompactionBackgroundJob::CompactOp::doCompact(rocksdb::DB* db) const {
        ...
        LOG(1) << "starting compaction of range: "
              ...
              << " in cf: " << cfHandle->GetName()
              << " (_rangeDropped is " << _rangeDropped << ")";

        if (_rangeDropped) {
            auto s = rocksdb::DeleteFilesInRange(db, cfHandle, start, end);
            if (!s.ok()) {
        ...
        auto s = db->CompactRange(compact_options, cfHandle, start, end);
        if (!s.ok()) {
    ...
    void RocksCompactionScheduler::reportSkippedDeletionsAboveThreshold(rocksdb::ColumnFamilyHandle* cfHandle,
    ...
    Status RocksCompactionScheduler::compactAll(rocksdb::ColumnFamilyHandle* cfHandle) {
        return compactRange(cfHandle, std::string(), std::string());
    }

    Status RocksCompactionScheduler::compactRange(rocksdb::ColumnFamilyHandle* cfHandle,
                                                  const std::string& start, const std::string& end) {
        return _compactionJob->scheduleCompactOp(cfHandle, start, end);
    }

    Status RocksCompactionScheduler::compactPrefix(rocksdb::ColumnFamilyHandle* cfHandle,
                                                   const std::string& prefix) {
        return compactRange(cfHandle, prefix, rocksGetNextPrefix(prefix));
    }

    Status RocksCompactionScheduler::compactDroppedRange(rocksdb::ColumnFamilyHandle* cfHandle,
                                                         const std::string& start, const std::string& end,
                                                         const std::function<void(bool)>& cleanup) {
        return _compactionJob->scheduleCompactOp(cfHandle, start, end, true, cleanup);
    }

    Status RocksCompactionScheduler::compactDroppedPrefix(rocksdb::ColumnFamilyHandle* cfHandle,
                                                          const std::string& prefix,
                                                          const std::function<void(bool)>& cleanup) {
        return compactDroppedRange(cfHandle, prefix, rocksGetNextPrefix(prefix), cleanup);
    }
