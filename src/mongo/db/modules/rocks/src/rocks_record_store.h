...
namespace mongo {
    ...
    public:
        ...
        // used for update numRecords & dataSize when delete oplog sst
        void updateStats(OperationContext* txn, long long numRecordsInc, long long dataSizeInc);
    ...
    private:
        ...
        static RecordData _getDataFor(rocksdb::DB* db, rocksdb::ColumnFamilyHandle* cfHandle,
                                      const std::string& prefix, OperationContext* txn, const RecordId& loc);
    ...
    RecordStore* CreateRocksRecordStoreProxy(RocksRecordStore*, std::weak_ptr<RocksRecordStore> *out);
}
