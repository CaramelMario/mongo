...
namespace mongo {
    ...
    inline size_t rocksGetSstIndex(const std::string& sst) {
        dassert(sst.size() > 4); // xxxxxx.sst
        dassert(sst.substr(sst.size() - 4) == ".sst");
        int pos = (int) sst.size() - 5;
        while(std::isdigit(sst[pos])) {
            --pos;
        }
        return (size_t) std::atoi(&sst[pos + 1]);
    };
