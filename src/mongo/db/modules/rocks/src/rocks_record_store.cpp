...
namespace mongo {
    ...
    public:
        RocksOplogKeyTracker(rocksdb::ColumnFamilyHandle *cfHandle, std::string prefix)
            : _prefix(std::move(prefix)), _cfHandle(cfHandle) {}
        void insertKey(RocksRecoveryUnit* ru, const RecordId& loc, int len) {
            ...
            ru->writeBatch()->Put(_cfHandle, RocksRecordStore::_makePrefixedKey(_prefix, loc),
        ...
        rocksdb::Iterator* newIterator(RocksRecoveryUnit* ru) {
            return ru->NewIterator(_cfHandle, _prefix, true);
        }
    ...
    RocksRecordStore::RocksRecordStore(StringData ns, StringData id, rocksdb::DB* db,
        ...
        : RecordStore(ns),
          ...
          _oplogKeyTracker(!rocksGlobalOptions.oplogChunkWiseDelete && _isOplog
                           ? new RocksOplogKeyTracker(cfHandle, rocksGetNextPrefix(_prefix))
                           : nullptr),
        ...
        std::unique_ptr<RocksIterator> iter(
            RocksRecoveryUnit::NewIteratorNoSnapshot(_db, _cfHandle, _prefix));
    ...
    RecordData RocksRecordStore::dataFor(OperationContext* txn, const RecordId& loc) const {
        RecordData rd = _getDataFor(_db, _cfHandle, _prefix, txn, loc);
    ...
    void RocksRecordStore::deleteRecord( OperationContext* txn, const RecordId& dl ) {
        ...
        ru->writeBatch()->Delete(_cfHandle, key);
        if (_oplogKeyTracker) {
    ...
    int64_t RocksRecordStore::cappedDeleteAsNeeded(OperationContext* txn,
        ...
        if (_isOplog && rocksGlobalOptions.oplogChunkWiseDelete) {
          return 0;
        }
    ...
    int64_t RocksRecordStore::cappedDeleteAsNeeded_inlock(OperationContext* txn,
        ...
        try {
            ...
            if (_oplogKeyTracker) {
                ...
                iter.reset(ru->NewIterator(_cfHandle, _prefix));
            }
            int64_t storage;
        ...
        if (_oplogKeyTracker && rocksGlobalOptions.enableAutoCompactOplog) {
            ...
            (_oplogKeyTracker->getDeletedSinceCompaction() >= kOplogCompactEveryDeletedRecords)) {
                ...
                _compactionScheduler->compactRange(_cfHandle, _prefix, oldestAliveKey);
                ...
                _compactionScheduler->compactRange(_cfHandle, oplogKeyTrackerPrefix, oldestAliveKey);
    ...
    StatusWith<RecordId> RocksRecordStore::insertRecord( OperationContext* txn,
        ...
        ru->writeBatch()->Put(_cfHandle, _makePrefixedKey(_prefix, loc), rocksdb::Slice(data, len));
        if (_oplogKeyTracker) {
    ...
    Status RocksRecordStore::updateRecord(OperationContext* txn, const RecordId& loc,
        ...
        ru->writeBatch()->Put(_cfHandle, key, rocksdb::Slice(data, len));
        if (_oplogKeyTracker) {
            _oplogKeyTracker->insertKey(ru, loc, len);
        }
    ...
    void RocksRecordStore::appendCustomStats( OperationContext* txn,
        ...
        if (_isOplog && rocksGlobalOptions.oplogChunkWiseDelete) {
            BSONObjBuilder oplogStones(result->subobjStart("oplogStones"));
            BSONArrayBuilder stones;

            rocksdb::TablePropertiesCollection prop;
            _db->GetPropertiesOfAllTables(_cfHandle, &prop);
            if (!prop.empty()) {
                typedef std::pair<const std::string*, const rocksdb::TableProperties*> info_t;
                std::vector<info_t> files;
                std::string prop_key = RocksEngine::kOplogCollectorPrefix;
                prop_key.append(_prefix.data(), _prefix.size());
                auto comp = [&](const info_t& l, const info_t& r) {
                    return rocksGetSstIndex(*l.first) < rocksGetSstIndex(*r.first);
                };
                for (auto &pair : prop) {
                    files.emplace_back(&pair.first, pair.second.get());
                }
                std::sort(files.begin(), files.end(), comp);
                for (auto& file : files) {
                    auto prop = file.second;
                    auto user_prop = prop->user_collected_properties;
                    auto find = user_prop.find(prop_key);
                    int64_t lastRecord;
                    if (find == user_prop.end()) {
                        continue;
                    }
                    else {
                        memcpy(&lastRecord, find->second.c_str(), 8);
                        lastRecord = endian::bigToNative(lastRecord);
                    }
                    size_t records = prop->num_entries;
                    size_t bytes = prop->raw_value_size;
                    stones.append(BSON("records" << static_cast<long long>(records)
                                                 << "bytes" << static_cast<long long>(bytes)
                                                 << "lastOpTime" << static_cast<int>(lastRecord >> 32)));
                }
            }
            oplogStones.append("stones", stones.arr());
        }
    }
    ...
    boost::optional<RecordId> RocksRecordStore::oplogStartHack(
        ...
        if (_oplogKeyTracker) {
            ...
            iter.reset(ru->NewIterator(_cfHandle, _prefix, true));
        }
        int64_t storage;
    ...
    void RocksRecordStore::temp_cappedTruncateAfter( OperationContext* txn,
        ...
        wuow.commit();
        if (_isOplog && rocksGlobalOptions.oplogChunkWiseDelete) {
            std::unique_lock<std::mutex> oplogLock(oplogMutex);
            log() << "RocksRecordStore rollback oplog ...";
            _db->Flush(rocksdb::FlushOptions(), _cfHandle);
            rocksdb::ColumnFamilyMetaData meta;
            _db->GetColumnFamilyMetaData(_cfHandle, &meta);
            std::vector<std::string> files;
            files.reserve(meta.levels[0].files.size());
            for (auto& file : meta.levels[0].files) {
                files.emplace_back(file.name);
            }
            _db->CompactFiles(rocksdb::CompactionOptions(), _cfHandle, files, 0);
        }
    }
    ...
    boost::optional<Record> RocksRecordStore::Cursor::curr() {
        //invariantRocksOK(_iterator->status());    // this call makes merging iterator check all children status ...
        if (!_iterator->Valid()) {
            _eof = true;
            return {};
        }
    ...
    class RocksRecordStoreProxy : public RecordStore {
        MONGO_DISALLOW_COPYING(RocksRecordStoreProxy);
    public:
        RocksRecordStoreProxy(RocksRecordStore* rsPtr)
            : RecordStore(std::string())
            , _p(rsPtr)
        {}

        const char* name() const override { return _p->name(); }
        const std::string& ns() const override { return _p->ns(); }
        long long dataSize(OperationContext* txn) const override { return _p->dataSize(txn); }
        long long numRecords(OperationContext* txn) const override { return _p->numRecords(txn); }
        bool isCapped() const override { return _p->isCapped(); }
        void setCappedCallback(CappedCallback* callback) override { _p->setCappedCallback(callback); }
        int64_t storageSize(OperationContext* txn,
                            BSONObjBuilder* extraInfo = NULL,
                            int infoLevel = 0) const override {
            return _p->storageSize(txn, extraInfo, infoLevel);
        }
        RecordData dataFor(OperationContext* txn, const RecordId& loc) const override {
            return _p->dataFor(txn, loc);
        }
        bool findRecord(OperationContext* txn, const RecordId& loc, RecordData* out) const override {
            return _p->findRecord(txn, loc, out);
        }
        void deleteRecord(OperationContext* txn, const RecordId& dl) override {
            _p->deleteRecord(txn, dl);
        }
        StatusWith<RecordId> insertRecord(OperationContext* txn,
                                          const char* data,
                                          int len,
                                          bool enforceQuota) override {
            return _p->insertRecord(txn, data, len, enforceQuota);
        }
        Status insertRecords(OperationContext* txn,
                             std::vector<Record>* records,
                             bool enforceQuota) override {
            return _p->insertRecords(txn, records, enforceQuota);
        }
        Status insertRecordsWithDocWriter(OperationContext* txn,
                                          const DocWriter* const* docs,
                                          size_t nDocs,
                                          RecordId* idsOut = nullptr) override {
            return _p->insertRecordsWithDocWriter(txn, docs, nDocs, idsOut);
        }
        Status updateRecord(OperationContext* txn,
                            const RecordId& oldLocation,
                            const char* data,
                            int len,
                            bool enforceQuota,
                            UpdateNotifier* notifier) override {
            return _p->updateRecord(txn, oldLocation, data, len, enforceQuota, notifier);
        }
        bool updateWithDamagesSupported() const override { return _p->updateWithDamagesSupported(); }
        StatusWith<RecordData> updateWithDamages(OperationContext* txn,
                                                 const RecordId& loc,
                                                 const RecordData& oldRec,
                                                 const char* damageSource,
                                                 const mutablebson::DamageVector& damages) override {
            return _p->updateWithDamages(txn, loc, oldRec, damageSource, damages);
        }
        std::unique_ptr<SeekableRecordCursor> getCursor(OperationContext* txn,
                                                        bool forward = true) const override {
            return _p->getCursor(txn, forward);
        }
        std::unique_ptr<RecordCursor> getCursorForRepair(OperationContext* txn) const override {
            return _p->getCursorForRepair(txn);
        }
        std::unique_ptr<RecordCursor> getRandomCursor(OperationContext* txn) const override {
            return _p->getRandomCursor(txn);
        }
        std::vector<std::unique_ptr<RecordCursor>> getManyCursors(OperationContext* txn) const override {
            return _p->getManyCursors(txn);
        }
        Status truncate(OperationContext* txn) override { return _p->truncate(txn); }
        void temp_cappedTruncateAfter(OperationContext* txn, RecordId end, bool inclusive) override {
            _p->temp_cappedTruncateAfter(txn, end, inclusive);
        }
        bool compactSupported() const override { return _p->compactSupported(); }
        bool compactsInPlace() const override { return _p->compactsInPlace(); }
        Status compact(OperationContext* txn,
                       RecordStoreCompactAdaptor* adaptor,
                       const CompactOptions* options,
                       CompactStats* stats) override {
            return _p->compact(txn, adaptor, options, stats);
        }
        Status validate(OperationContext* txn,
            ...
            return _p->validate(txn, level, adaptor, results, output);
        }
        void appendCustomStats(OperationContext* txn,
                               BSONObjBuilder* result,
                               double scale) const override {
            _p->appendCustomStats(txn, result, scale);
        }
        Status touch(OperationContext* txn, BSONObjBuilder* output) const override {
            return _p->touch(txn, output);
        }
        boost::optional<RecordId> oplogStartHack(OperationContext* txn,
                                                 const RecordId& startingPosition) const override {
            return _p->oplogStartHack(txn, startingPosition);
        }
        Status oplogDiskLocRegister(OperationContext* txn, const Timestamp& opTime) override {
            return _p->oplogDiskLocRegister(txn, opTime);
        }
        void waitForAllEarlierOplogWritesToBeVisible(OperationContext* txn) const override {
            _p->waitForAllEarlierOplogWritesToBeVisible(txn);
        }
        void updateStatsAfterRepair(OperationContext* txn,
                                    long long numRecords,
                                    long long dataSize) override {
            _p->updateStatsAfterRepair(txn, numRecords, dataSize);
        }

        std::weak_ptr<RocksRecordStore> getRecordStorePtr() { return _p; }

    protected:
        std::shared_ptr<RocksRecordStore> _p;
    };

    RecordStore* CreateRocksRecordStoreProxy(RocksRecordStore* rsPtr, std::weak_ptr<RocksRecordStore> *out) {
        auto proxy = new RocksRecordStoreProxy(rsPtr);
        if (out) {
            *out = proxy->getRecordStorePtr();
        }
        return proxy;
    }
}
