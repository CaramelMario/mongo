...
namespace rocksdb {
    // from rocksdb/db/internal_stats.cc
...
namespace mongo {
    ...
    namespace {
        ...
        BSONObj convertPlain2Bson(const std::vector<std::string>& arr, int start, int& index) {
            ...
            for (int i = start; i < (int)arr.size(); ) {
                ...
                if (indent > 0 && !isspace(str[indent - 1])) { // step to outside scope
                    ...
                    if (!right.empty()) { // same scope
                        ...
                        builder.append(trim_copy(left), convertPlain2Bson(arr, i + 1, index));
                        i = index;
                    }
                }
            }
    ...
    BSONObj RocksServerStatusSection::generateSection(OperationContext* txn,
        ...
        {
          BSONObjBuilder countersObjBuilder;
          rocksdb::ForEachInternalStatsKeys([&](std::string const& key) {
            if (key == "stats") {
                return;
            }
            std::string statsString;
            if (!_engine->getDB()->GetProperty(key, &statsString)) {
                statsString = "<error> unable to retrieve statistics";
            }
            if (statsString.find('\n') != std::string::npos) {
                BSONArrayBuilder a;
                ...
                countersObjBuilder.appendArray(key, a.arr());
            }
            else {
                countersObjBuilder.append(key, statsString);
            }
          });
          bob.append("detail", countersObjBuilder.obj());
        }
        ...
        if (tableFactory) {
          ...
          if (!settings.empty()) {
              int index = 0;
              bob.append("table-options", convertPlain2Bson(settings, 0, index));
          }
        }
